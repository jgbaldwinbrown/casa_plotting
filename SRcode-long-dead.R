...
data = read.table ('deadsout_extracted.txt', sep = ' ') 
...
g = ggplot(data = mdetailed[mdetailed$variable == "VCL" & mdetailed$extracted != "pure", ], aes(x=segnum, y=value)) + 
  ...
  scale_linetype_manual(values = c("solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid"), guide="none") +
  theme_bw() +
...
pdf("VCL_long_dead.pdf", height = 3 * scale, width = 4 * scale)
print(g)
dev.off()
...
scale = 2.0
pdf("VSL_long_dead.pdf", height = 3 * scale, width = 4 * scale)
print(g)
dev.off()

## Generating VAP Plots

g = ggplot(data = mdetailed[mdetailed$variable == "VAP" & mdetailed$extracted != "pure" & mdetailed$segnum < 4.5, ], aes(x=segnum, y=value)) + 
  geom_boxplot(aes(fill = prettyextracted, linetype = factor(segnum))) +
  scale_linetype_manual(values = c("solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid"), guide="none") +
  theme_bw() +
  geom_smooth(method = "lm", aes(color = prettyextracted)) +
...
print(levels(factor(mdetailed$extracted)))
print(levels(factor(mdetailed$segnum)))


pdf("VAP_long_dead.pdf", height = 3 * scale, width = 4 * scale)
print(g)
dev.off()
...
pdf("LIN_long_dead.pdf", height = 3 * scale, width = 4 * scale)
print(g)
dev.off()
...
pdf("WOB_long_dead.pdf", height = 3 * scale, width = 4 * scale)
print(g)
dev.off()
...
pdf("NM_long_dead.pdf", height = 3 * scale, width = 4 * scale)
print(g)
dev.off()
